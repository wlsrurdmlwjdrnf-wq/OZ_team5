...
//   1) 행(rows)을 자동 추정
//   2) 타일 간격(spacingY)을 자동 추정
//   3) 전체 높이(height = rows*spacingY)를 계산
//- 각 타일에 대해 : 플레이어가 타일 중심에서 height/2 이상 멀어지면 타일을 height만큼 반대편으로 이동
public class UpDownReposition : MonoBehaviour
{
    [Header("References")]
    ...
    [Header("Tolerance")]
    //Ground들의 중심 좌표를 "같은 행"으로 묶을 때 사용하는 허용 오차
    //프리팹 배치 시 약간의 오차가 있어도 정상적으로 격자를 인식하게 하기 위한 안전 장치
    //있으면 미래의 버그를 미리 막아준다
    [Tooltip("타일 중심 y 값들을 같은 행으로 묶을 때 사용하는 허용 오차")]
    ...
    private void LateUpdate()
    {
        ...
        for (int i = 0; i < grounds.Length; i++)
        {
            ...
            //플레이어가 타일보다 충분히 위에 있으면( dy > +height/2 ) 타일을 위로 올림
            //플레이어가 타일보다 충분히 아래에 있으면( dy < -height/2 ) 타일을 아래로 내림
            float dy = p.y - gp.y;
    ...
    //현재 grounds 배치로부터 rows, spacingY, height를 자동 추정
    private bool TryComputeGridInfo()
    {
        ...
        if (ys.Count < 2) return false;

        //2)y를 "비슷한 값끼리" 묶어서 고유 행 개수 추정
        ...
        //rows가 1이면 spacing이 0이 될 수 있음 → bounds로 보정 시도
        if (rows == 1) spacingY = EstimateSizeFromBoundsY();

        if (spacingY <= 0f) return false;
